...
public class StuNoRegist extends HttpServlet 
{
...
"</tr>");
            ResultSet qs= st.executeQuery("select sid,sname from stu where sid not in(select sid from regis) and branch="+"'"+bn+"'"+"and years="+"'"+ye+"'"+"and sem="+"'"+sem+"'");
            while(qs.next())
            {
                out.println("<tr>\n" +
